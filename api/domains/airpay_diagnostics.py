...
from domains.airpay_preparation import build_check_payload
...
class AirpayDiagnosticStore:
    ...
    def read(self, owner, run_id=None, conn=None):
        ...
        run = conn.execute('SELECT * FROM app.airpay_diagnostic_runs WHERE created_by=%s ' +
        ...
        if not run:
            ...
            return {'run': None}
        rows = conn.execute('SELECT * FROM app.airpay_diagnostic_items WHERE run_id=%s ORDER BY position', (run['id'],)).fetchall()
...
class AirpayDiagnostics:
    ...
    def start(self, owner, run_id):
        ...
        with self.store.locked() as conn:
            exists = conn.execute('SELECT id FROM app.airpay_diagnostic_runs WHERE id=%s', (run_id,)).fetchone()
            if exists:
                return self.store.read(owner, run_id, conn)
            if conn.execute("SELECT id FROM app.airpay_diagnostic_runs WHERE state='active'").fetchone():
            ...
            with conn.transaction():
                conn.execute('INSERT INTO app.airpay_diagnostic_runs(id,created_by) VALUES (%s,%s)', (run_id, owner))
                for index, service in enumerate(catalog['items']):
                    conn.execute('INSERT INTO app.airpay_diagnostic_items(run_id,position,service_id,title) VALUES (%s,%s,%s,%s)',
    ...
    def cancel(self, owner, run_id):
        ...
        with self.store.locked() as conn:
            ...
            conn.execute("UPDATE app.airpay_diagnostic_runs SET state='cancelled',updated_at=now() WHERE id=%s AND state='active'", (run_id,))
            return self.store.read(owner, run_id, conn)
    ...
    def step(self, owner, run_id):
        ...
        with self.store.locked() as conn:
            ...
            if report['run']['state'] != 'active':
                return report
            conn.execute("""UPDATE app.airpay_diagnostic_items SET state='interrupted',finished_at=now(),
                report='{"message":"Предыдущий шаг прерван; check автоматически не повторялся"}'::jsonb
                WHERE run_id=%s AND state='running'""", (run_id,))
            if conn.execute("SELECT 1 FROM app.airpay_diagnostic_items WHERE run_id=%s AND finished_at > now()-interval '1 second'", (run_id,)).fetchone():
                return self.store.read(owner, run_id, conn)
            row = conn.execute("SELECT * FROM app.airpay_diagnostic_items WHERE run_id=%s AND state='pending' ORDER BY position LIMIT 1", (run_id,)).fetchone()
            if row:
                agent_id = secrets.randbelow(9223372036854775806) + 1
                conn.execute("UPDATE app.airpay_diagnostic_items SET state='running',started_at=now(),agent_transaction_id=%s WHERE run_id=%s AND position=%s", (agent_id, run_id, row['position']))
                try:
                ...
                conn.execute('UPDATE app.airpay_diagnostic_items SET state=%s,report=%s,finished_at=now() WHERE run_id=%s AND position=%s',
                             (result['state'], Jsonb(result), run_id, row['position']))
            conn.execute("""UPDATE app.airpay_diagnostic_runs SET updated_at=now(),state=CASE WHEN EXISTS
                (SELECT 1 FROM app.airpay_diagnostic_items WHERE run_id=%s AND state='pending') THEN 'active' ELSE 'completed' END WHERE id=%s""", (run_id, run_id))
            return self.store.read(owner, run_id, conn)
