...
class AirpayJobStore:
    ...
    def enqueue(self, owner, transaction_id, action, payload):
        ...
        with self.connect(autocommit=True, row_factory=dict_row) as conn:
            row = conn.execute('''INSERT INTO app.airpay_jobs(id,created_by,transaction_id,action,payload)
                VALUES (%s,%s,%s,%s,%s) ON CONFLICT (created_by,transaction_id)
                WHERE state IN ('queued','running') DO UPDATE SET updated_at=app.airpay_jobs.updated_at RETURNING *''',
    ...
    def get(self, owner, job_id):
        ...
        with self.connect(autocommit=True, row_factory=dict_row) as conn:
            row = conn.execute('SELECT * FROM app.airpay_jobs WHERE id=%s AND created_by=%s', (job_id, owner)).fetchone()
        if not row:
    ...
    def active(self, owner, transaction_id):
        ...
        with self.connect(autocommit=True, row_factory=dict_row) as conn:
            row = conn.execute("SELECT * FROM app.airpay_jobs WHERE created_by=%s AND transaction_id=%s AND state IN ('queued','running')", (owner, transaction_id)).fetchone()
    ...
    def cancel(self, owner, job_id):
        ...
        with self.connect(autocommit=True, row_factory=dict_row) as conn:
            ...
            try:
                ...
                row = conn.execute("UPDATE app.airpay_jobs SET state='failed',error=%s,error_status=409,updated_at=now() WHERE id=%s RETURNING *",
    ...
    def latest(self, owner, transaction_id):
        ...
        with self.connect(autocommit=True, row_factory=dict_row) as conn:
            row = conn.execute('SELECT * FROM app.airpay_jobs WHERE created_by=%s AND transaction_id=%s ORDER BY created_at DESC,id DESC LIMIT 1',
    ...
    def candidates(self, conn, actions, owner):
        ...
        while True:
            rows = conn.execute("""SELECT id,created_at FROM app.airpay_jobs
        ...
        with self.connect(autocommit=True, row_factory=dict_row) as conn:
            for item in self.candidates(conn, actions, owner):
                ...
                if not acquired:
                    continue
                try:
                    row = conn.execute("SELECT * FROM app.airpay_jobs WHERE id=%s AND state IN ('queued','running')", (item['id'],)).fetchone()
                    if not row:
                        continue
                    if row['state'] == 'running' and row['action'] in SENSITIVE_ACTIONS:
                        ...
                        conn.execute("UPDATE app.airpay_jobs SET state='failed',error=%s,error_status=409,updated_at=now() WHERE id=%s",
                                     (INTERRUPTED, row['id']))
                        logging.getLogger(__name__).warning('Airpay interrupted job stopped: id=%s action=%s', row['id'], row['action'])
                        return True
                    if row['action'] not in actions:
                        continue
                    conn.execute("UPDATE app.airpay_jobs SET state='running',updated_at=now() WHERE id=%s", (row['id'],))
                    def progress(value):
                        # Прогресс фиксируется после каждой позиции и читается независимо от сети поставщика.
                        conn.execute('UPDATE app.airpay_jobs SET progress=%s,updated_at=now() WHERE id=%s', (value, row['id']))
                    try:
                        result = execute(row, progress)
                    except HTTPException as exc:
                        conn.execute("UPDATE app.airpay_jobs SET state='failed',error=%s,error_status=%s,updated_at=now() WHERE id=%s", (str(exc.detail)[:2000], exc.status_code, row['id']))
                    except Exception:
                        ...
                        conn.execute("UPDATE app.airpay_jobs SET state='failed',error='Запрос прерван. Обновите сохранённый результат покупки.',error_status=503,updated_at=now() WHERE id=%s", (row['id'],))
                    else:
                        conn.execute("UPDATE app.airpay_jobs SET state='succeeded',result=%s,updated_at=now() WHERE id=%s", (Jsonb(jsonable_encoder(hide_codes(result))), row['id']))
                    return True
                finally:
                    conn.execute('SELECT pg_advisory_unlock(hashtextextended(%s,0))', (lock_name,))
        return False
