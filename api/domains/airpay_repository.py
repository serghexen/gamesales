...
class AirpayRepository:
    ...
    def create(self, owner, key, request, service, kind, expires_at):
        ...
        with self.connect(autocommit=True, row_factory=dict_row) as conn:
            conn.execute('''INSERT INTO app.airpay_transactions
            ...
            row = conn.execute('SELECT * FROM app.airpay_transactions WHERE created_by=%s AND preparation_key=%s', (owner, key)).fetchone()
    ...
    def locked(self, owner, transaction_id):
        ...
        with self.connect(autocommit=True, row_factory=dict_row) as conn:
            ...
            try:
                row = conn.execute('SELECT * FROM app.airpay_transactions WHERE agent_transaction_id=%s AND created_by=%s',
    ...
    def batch_rows(self, owner, root):
        ...
        with self.connect(autocommit=True, row_factory=dict_row) as conn:
            rows = conn.execute('SELECT * FROM app.airpay_transactions WHERE created_by=%s AND preparation_key = ANY(%s::uuid[])',
                                (owner, keys)).fetchall()
    ...
    def batch_locked(self, owner, transaction_id):
        ...
        with self.connect(autocommit=True, row_factory=dict_row) as conn:
            root = conn.execute('SELECT * FROM app.airpay_transactions WHERE agent_transaction_id=%s AND created_by=%s',
                                (transaction_id, owner)).fetchone()
            if not root or not root['service_snapshot'].get('_airpay_batch'):
                raise HTTPException(404, 'Пачка Airpay не найдена')
            key = root['service_snapshot']['_airpay_batch']['key']
            root = conn.execute('SELECT * FROM app.airpay_transactions WHERE preparation_key=%s AND created_by=%s',
                                (key, owner)).fetchone()
            if not root:
                raise HTTPException(404, 'Пачка Airpay не найдена')
    ...
    def history(self, owner, limit, offset, filters=None):
        ...
        from domains.airpay_history import HistoryFilters, history_where
        where, params = history_where(owner, filters or HistoryFilters())
        with self.connect(autocommit=True, row_factory=dict_row) as conn:
            return [stored_row(row) for row in conn.execute(f'SELECT * FROM app.airpay_transactions WHERE {where} ORDER BY created_at DESC, agent_transaction_id DESC LIMIT %s OFFSET %s',
    ...
    def cutover_status(self):
        ...
        with self.connect(row_factory=dict_row) as conn:
            row = conn.execute('''SELECT
                (SELECT count(*) FROM app.airpay_transactions WHERE state='processing') AS pending_payments,
                (SELECT count(*) FROM app.airpay_transactions
                 WHERE state='paid' AND purchase_kind='voucher'
                   AND pin_ciphertext IS NULL AND COALESCE(btrim(pin_code),'')='') AS missing_vouchers,
                (SELECT count(*) FROM app.airpay_jobs WHERE state IN ('queued','running')) AS active_jobs
                ''').fetchone()
    ...
    def read(self, owner, transaction_id):
        ...
        with self.connect(autocommit=True, row_factory=dict_row) as conn:
            row = conn.execute('SELECT * FROM app.airpay_transactions WHERE agent_transaction_id=%s AND created_by=%s', (transaction_id, owner)).fetchone()
        if not row:
            raise HTTPException(404, 'Операция Airpay не найдена')
        return stored_row(row)
    ...
    def batch_root(self, owner, transaction_id):
        ...
        if not key:
            raise HTTPException(404, 'Пачка Airpay не найдена')
        with self.connect(autocommit=True, row_factory=dict_row) as conn:
            root = conn.execute('SELECT * FROM app.airpay_transactions WHERE preparation_key=%s AND created_by=%s', (key, owner)).fetchone()
        if not root:
            raise HTTPException(404, 'Пачка Airpay не найдена')
    ...
    def renew(self, owner, rows):
        ...
        with self.connect(row_factory=dict_row) as conn:
            existing = conn.execute('SELECT * FROM app.airpay_renewals WHERE source_id=%s AND created_by=%s', (rows[0]['agent_transaction_id'], owner)).fetchone()
            if existing:
                return existing
            ...
            key = uuid.uuid4()
            result = conn.execute('''INSERT INTO app.airpay_renewals(source_id,preparation_key,created_by,quantity,source_transaction_id)
                VALUES (%s,%s,%s,%s,%s) RETURNING *''', (rows[0]['agent_transaction_id'], key, owner, len(pending), pending[0]['agent_transaction_id'])).fetchone()
            conn.execute('UPDATE app.airpay_transactions SET replacement_key=%s,updated_at=now() WHERE agent_transaction_id=ANY(%s)', (key, [row['agent_transaction_id'] for row in pending]))
            return result
    ...
    def finish_renewal(self, owner, source_id, replacement_id):
        ...
        with self.connect(autocommit=True) as conn:
            conn.execute('UPDATE app.airpay_renewals SET replacement_id=%s WHERE source_id=%s AND created_by=%s', (replacement_id, source_id, owner))
    ...
    def replacement(self, owner, source_id):
        ...
        with self.connect(autocommit=True, row_factory=dict_row) as conn:
            row = conn.execute('SELECT replacement_id FROM app.airpay_renewals WHERE source_id=%s AND created_by=%s', (source_id, owner)).fetchone()
    ...
    def reveal(self, owner, transaction_id):
        ...
        with self.connect(row_factory=dict_row) as conn:
            row = conn.execute("SELECT CASE WHEN pin_ciphertext IS NOT NULL THEN pgp_sym_decrypt(pin_ciphertext,%s) ELSE pin_code END AS pin_code FROM app.airpay_transactions WHERE agent_transaction_id=%s AND created_by=%s AND state='paid'", (self.code_secret(), transaction_id, owner)).fetchone()
            if not row or not row['pin_code']:
                raise HTTPException(404, 'Сохранённый код не найден')
            conn.execute('INSERT INTO app.airpay_result_access(transaction_id,viewed_by) VALUES (%s,%s)', (transaction_id, owner))
    ...
    def events(self, owner, transaction_id, before=None, limit=50):
        ...
        with self.connect(row_factory=dict_row) as conn:
            rows = conn.execute('''SELECT e.id::text,e.event_type,e.state_before,e.state_after,e.actor,e.details,e.created_at,
                a.decision,a.evidence FROM app.airpay_events e
                LEFT JOIN app.airpay_operator_actions a ON a.transaction_id=e.transaction_id
    ...
    def resolve(self, owner, transaction_id, payload):
        ...
        with self.locked(owner, transaction_id) as record:
            with record.conn.transaction():
                existing = record.conn.execute('''SELECT request_hash FROM app.airpay_operator_actions
                ...
                record.conn.execute('''INSERT INTO app.airpay_operator_actions
...
class AirpayRecord:
    ...
    def update(self, **changes):
        ...
        self.row = stored_row(self.conn.execute('UPDATE app.airpay_transactions SET ' + ', '.join(assignments)
