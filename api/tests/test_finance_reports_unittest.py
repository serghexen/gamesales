...
class FinanceReportsTests(unittest.TestCase):
    ...
    def test_finance_cash_flow_report_success(self):
        ...
        self.assertTrue(any("FROM app.accounts a" in sql for sql in sql_collector))
        self.assertTrue(any("CONCAT('Закуп Шеринг '" in sql for sql in sql_collector))
        self.assertTrue(any("'indirect'::text AS expense_kind" in sql for sql in sql_collector))
        self.assertTrue(any("a.purchase_cost * COALESCE(r.purchase_cost_rate, 1.0)" in sql for sql in sql_collector))
    ...
    def test_finance_cash_flow_details_success(self):
        ...
        self.assertTrue(any("'account'::text AS row_type" in sql for sql in sql_collector))
        self.assertTrue(any("Закупочная цена аккаунта с коэффициентом региона" in sql for sql in sql_collector))
