...
ALTER TABLE app.airpay_transactions ADD COLUMN IF NOT EXISTS requires_attention boolean NOT NULL DEFAULT false;
ALTER TABLE app.airpay_transactions ADD COLUMN IF NOT EXISTS attention_reason text NOT NULL DEFAULT '';
ALTER TABLE app.airpay_transactions ADD COLUMN IF NOT EXISTS resolution_request_id uuid;

CREATE TABLE IF NOT EXISTS app.airpay_operator_actions (
    transaction_id bigint NOT NULL REFERENCES app.airpay_transactions(agent_transaction_id),
...
CREATE TABLE IF NOT EXISTS app.airpay_events (
...
CREATE OR REPLACE FUNCTION app.airpay_record_event() RETURNS trigger LANGUAGE plpgsql AS $$
...
BEGIN
    ...
    INSERT INTO app.airpay_events(transaction_id,event_type,state_before,state_after,actor,details)
...
END;
$$;
CREATE TRIGGER airpay_transaction_event AFTER INSERT OR UPDATE ON app.airpay_transactions
FOR EACH ROW EXECUTE FUNCTION app.airpay_record_event();
