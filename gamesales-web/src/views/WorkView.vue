<template>
  <div class="page">
    ...
    <div class="shell">
        ...
        <div class="actions">
          <nav class="tabs">
            ...
            <router-link
              ...
              :to="{ name: 'work', query: { ...route.query, tab: 'telegram' } }"
            >
              Чаты
            ...
            <router-link
              ...
              :to="{ name: 'work', query: { ...route.query, tab: 'analytics' } }"
            >
              Аналитика
      ...
      <section v-if="showDashboard && activeTab === 'dashboard'" class="hero">
        <div>
      ...
      <main class="main">
        <section v-if="showDashboard && activeTab === 'dashboard'" class="panel panel--wide">
          ...
          <div class="panel__body">
            ...
            <div v-else class="tg-shell">
              <div class="tg-dialogs">
                <div class="tg-dialogs__head">
                  <div class="tg-dialog-tabs" role="tablist" aria-label="Статусы диалогов">
                    ...
                    >
                      Новые
                      <span v-if="telegram.dialogCounts.new" class="tg-dialog-tab__count">{{ telegram.dialogCounts.new }}</span>
                    </button>
                    <button
                      class="tg-dialog-tab"
                      :class="{ active: telegram.dialogStatusFilter === 'accepted' }"
                      type="button"
                      @click="setTelegramDialogFilter('accepted')"
                    >
                      Принятые
                      <span v-if="telegram.dialogCounts.accepted" class="tg-dialog-tab__count">{{ telegram.dialogCounts.accepted }}</span>
                    </button>
                    <button
                      class="tg-dialog-tab"
                      :class="{ active: telegram.dialogStatusFilter === 'archived' }"
                      type="button"
                      @click="setTelegramDialogFilter('archived')"
                    >
                      Архив
              ...
              <div class="tg-messages">
                <div v-if="telegram.loadingMessages" class="tg-loading-overlay">
                  <div class="loader-wrap loader-wrap--compact">
                    <div class="newtons-cradle" aria-label="Loading" role="img">
                      <div class="newtons-cradle__dot"></div>
                      <div class="newtons-cradle__dot"></div>
                ...
                <div class="tg-contact">
                  <div class="tg-contact__head">
                    ...
                    <div class="tg-contact__tools">
                      <button
                        v-if="telegram.activeDialog && telegram.activeDialog.status !== 'accepted'"
                        class="ghost ghost--tiny"
                        type="button"
                        @click="setTelegramDialogStatus('accepted')"
                        :disabled="telegram.loading || !telegram.activeChatId"
                      >
                        Принять
                      </button>
                      <button
                        v-if="telegram.activeDialog && telegram.activeDialog.status !== 'archived'"
                        class="ghost ghost--tiny"
                        type="button"
                        @click="setTelegramDialogStatus('archived')"
                        :disabled="telegram.loading || !telegram.activeChatId"
                      >
                        В архив
                      </button>
                ...
                <TransitionGroup
                  ...
                  @scroll="onTelegramMessagesScroll"
                >
                  <div v-if="telegram.loadingOlderMessages" key="__loading_older__" class="tg-messages__older-loading">
                  ...
                  >
                    ...
                    >
                      <div class="tg-message__text" v-html="formatTelegramMessageHtml(m.text || '—')"></div>
                      ...
                      <div v-if="m.has_media" class="tg-message__media">
                        <img
                          v-if="isTelegramImage(m) && m.media_url"
                          :src="m.media_url"
                          :alt="m.media_type === 'gif' ? 'GIF' : 'Image'"
                          @load="onTelegramMediaRendered"
                        />
                        <video v-else-if="isTelegramVideo(m) && m.media_url" :src="m.media_url" controls @loadedmetadata="onTelegramMediaRendered" />
                      ...
                      <div v-if="showTelegramChannelLabel(idx)" class="tg-message__channel">
                        Получено из Telegram {{ telegram.activeDialog?.is_group ? 'Группа' : 'Личный' }} — ASAT
                      </div>
                    </div>
                ...
                <div class="tg-compose">
                  ...
                  <div class="tg-compose__body">
                    <textarea
                      v-model.trim="telegram.messageText"
                    ...
                    <div class="tg-compose__actions">
                      ...
                      <button
                        ...
                        :disabled="telegram.loading || !telegram.activeChatId || !telegram.messageText"
...
const TELEGRAM_DIALOGS_POLL_ERROR_MS = 15000
const TELEGRAM_MESSAGES_POLL_ERROR_MS = 3000
...
async function selectTelegramDialog(dialogId) {
  ...
  try {
    ...
    telegram.loadingMessages = false
    // Do not block the chat UI on media downloads.
...
function resolveTelegramMessagesEl() {
  ...
  // `ref` on TransitionGroup can be component instance instead of raw element.
...
function formatTelegramMessageHtml(value) {
...
.tg-dialog-tab__count {
  min-width: 16px;
  height: 16px;
  border-radius: 999px;
  background: rgba(58, 168, 86, 0.9);
  color: #fff;
  font-size: 10px;
  line-height: 16px;
  text-align: center;
...
.tg-messages__older-loading {
  align-self: center;
  font-size: 11px;
  color: #9ad8ff;
  background: rgba(40, 115, 180, 0.18);
...
.tg-message--out .tg-message__text a {
  color: #9dd2ff;
}
...
.tg-message__media-placeholder {
  ...
  animation: tgMediaShimmer 1.2s linear infinite;
}

@keyframes tgMediaShimmer {
  to {
...
.tg-message__channel {
  margin-top: 4px;
  font-size: 11px;
  color: rgba(168, 182, 214, 0.9);
}

.tg-message-row__actions,
.tg-message-row__actions-spacer {
  display: inline-flex;
  flex-direction: column;
  gap: 6px;
  width: 24px;
...
.tg-message-action__glyph {
  font-size: 12px;
  line-height: 1;
  font-weight: 700;
}

.tg-message-action--blue {
  color: #67b9ff;
}

.tg-message-action--red {
  color: #ff6c8a;
}

.tg-message-action--ghost {
  opacity: 0;
  pointer-events: none;
}
...
.tg-message-row:hover .tg-message-row__actions,
.tg-message-row:focus-within .tg-message-row__actions {
  visibility: visible;
  opacity: 1;
  pointer-events: auto;
}
...
.tg-compose {
  margin-top: 10px;
  border: 1px solid rgba(180, 189, 206, 0.45);
  border-radius: 12px;
  overflow: hidden;
  background: rgba(226, 231, 240, 0.08);
}
...
.tg-compose__head {
  ...
  gap: 8px;
  padding: 0 14px;
  background: rgba(196, 205, 220, 0.22);
  border-bottom: 1px solid rgba(180, 189, 206, 0.35);
}
...
.tg-compose__input::placeholder {
  color: rgba(195, 205, 220, 0.85);
}
...
.tg-compose__icon {
  ...
  height: 34px;
  border-radius: 999px;
  border: 1px solid rgba(180, 189, 206, 0.35);
  background: rgba(180, 189, 206, 0.12);
  color: #b8c3d8;
  display: inline-flex;
  align-items: center;
  justify-content: center;
...
.tg-compose__send {
  ...
  height: 38px;
  border-radius: 999px;
  border: 1px solid rgba(103, 185, 255, 0.4);
  background: rgba(103, 185, 255, 0.18);
  color: #79c5ff;
  display: inline-flex;
  align-items: center;
  justify-content: center;
...
.tg-compose__send:disabled {
  opacity: 0.45;
  cursor: not-allowed;
}

.tg-compose__send svg {
  width: 19px;
  height: 19px;
  stroke: currentColor;
  fill: currentColor;
...
.hamster {
  animation: gsHamsterRun var(--dur) ease-in-out infinite;
  top: 50%;
  left: calc(50% - 3.5em);
  width: 7em;
  height: 3.75em;
...
.hamster__head {
  animation: gsHamsterHead var(--dur) ease-in-out infinite;
...
.hamster__ear {
  animation: gsHamsterEar var(--dur) ease-in-out infinite;
...
.hamster__eye {
  animation: gsHamsterEye var(--dur) linear infinite;
...
.hamster__body {
  animation: gsHamsterBody var(--dur) ease-in-out infinite;
...
.hamster__limb--fr {
  animation: gsHamsterFRLimb var(--dur) linear infinite;
  background: linear-gradient(hsl(30, 90%, 80%) 80%, hsl(0, 90%, 75%) 80%);
  transform: rotate(15deg) translateZ(-1px);
}

.hamster__limb--fl {
  animation: gsHamsterFLLimb var(--dur) linear infinite;
...
.hamster__limb--br {
  animation: gsHamsterBRLimb var(--dur) linear infinite;
  background: linear-gradient(hsl(30, 90%, 80%) 90%, hsl(0, 90%, 75%) 90%);
  transform: rotate(-25deg) translateZ(-1px);
}

.hamster__limb--bl {
  animation: gsHamsterBLLimb var(--dur) linear infinite;
  background: linear-gradient(hsl(30, 90%, 90%) 90%, hsl(0, 90%, 85%) 90%);
  transform: rotate(-25deg);
}

.hamster__tail {
  animation: gsHamsterTail var(--dur) linear infinite;
  background: hsl(0, 90%, 85%);
...
.spoke {
  animation: gsHamsterSpoke var(--dur) linear infinite;
  background: radial-gradient(100% 100% at center, hsl(0, 0%, 60%) 4.8%, hsla(0, 0%, 60%, 0) 5%),
    linear-gradient(hsla(0, 0%, 55%, 0) 46.9%, hsl(0, 0%, 65%) 47% 52.9%, hsla(0, 0%, 65%, 0) 53%) 50% 50% / 99% 99% no-repeat;
}

@keyframes gsHamsterRun {
  from, to { transform: rotate(4deg) translate(-0.8em, 1.4em); }
  50% { transform: rotate(0) translate(-0.8em, 1.4em); }
}
@keyframes gsHamsterHead {
  from, 25%, 50%, 75%, to { transform: rotate(0); }
  12.5%, 37.5%, 62.5%, 87.5% { transform: rotate(8deg); }
}
@keyframes gsHamsterEye {
  from, 90%, to { transform: scaleY(1); }
  95% { transform: scaleY(0); }
}
@keyframes gsHamsterEar {
  from, 25%, 50%, 75%, to { transform: rotate(0); }
  12.5%, 37.5%, 62.5%, 87.5% { transform: rotate(12deg); }
}
@keyframes gsHamsterBody {
  from, 25%, 50%, 75%, to { transform: rotate(0); }
  12.5%, 37.5%, 62.5%, 87.5% { transform: rotate(-2deg); }
}
@keyframes gsHamsterFRLimb {
  from, 25%, 50%, 75%, to { transform: rotate(50deg) translateZ(-1px); }
  12.5%, 37.5%, 62.5%, 87.5% { transform: rotate(-30deg) translateZ(-1px); }
}
@keyframes gsHamsterFLLimb {
  from, 25%, 50%, 75%, to { transform: rotate(-30deg); }
  12.5%, 37.5%, 62.5%, 87.5% { transform: rotate(50deg); }
}
@keyframes gsHamsterBRLimb {
  from, 25%, 50%, 75%, to { transform: rotate(-60deg) translateZ(-1px); }
  12.5%, 37.5%, 62.5%, 87.5% { transform: rotate(20deg) translateZ(-1px); }
}
@keyframes gsHamsterBLLimb {
  from, 25%, 50%, 75%, to { transform: rotate(20deg); }
  12.5%, 37.5%, 62.5%, 87.5% { transform: rotate(-60deg); }
}
@keyframes gsHamsterTail {
  from, 25%, 50%, 75%, to { transform: rotate(30deg) translateZ(-1px); }
  12.5%, 37.5%, 62.5%, 87.5% { transform: rotate(10deg) translateZ(-1px); }
}
@keyframes gsHamsterSpoke {
...
.deal-form .input--compact {
  height: 30px;
  font-size: 12px;
}

.deal-form .input--textarea {
  min-height: 88px;
  padding: 8px 10px;
}
...
.deal-form .slot-status__list {
  gap: 4px;
}

.deal-form .quick-create {
  gap: 6px;
  padding: 8px;
}

.deal-form .quick-create--game-empty {
  margin-top: 6px;
}

.deal-form .quick-create .loader-wrap--compact {
  min-height: 84px;
  padding: 6px 0;
}

.deal-form .quick-create .wheel-and-hamster--mini {
  width: 6.8em;
  height: 6.8em;
  font-size: 9px;
}
...
.field--game .input--select-wrap--selected {
  width: 100%;
  max-width: 100%;
}

.deal-form__col--right .field--game,
.deal-form__col--right .field--game .input,
.deal-form__col--right .field--game .input--search,
.deal-form__col--right .field--game .input--select-wrap,
.deal-form__col--right .field--game .quick-create--game-empty {
  width: 100%;
  max-width: 100%;
  justify-self: stretch;
}
...
.deal-form .input--select-wrap--selected .input--list {
  min-height: 42px;
  height: 42px;
}
