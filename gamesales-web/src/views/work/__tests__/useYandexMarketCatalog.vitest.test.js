...
describe('WorkYandexMarketDigitalSettingsModal', () => {
  ...
  it('keeps production issue settings and the key pool in the separate keys modal', async () => {
    ...
    expect(wrapper.get('input[aria-label="Автовыдача через поставщика для Яндекс Маркета"]').attributes('disabled')).toBeUndefined()
    ...
    await wrapper.get('button[aria-label="Показать список товаров поставщика"]').trigger('click')
    expect(wrapper.get('#yandex-interhub-service-results').text()).toContain('PlayStation Turkey')
    expect(wrapper.get('select[aria-label="Номинал поставщика для Яндекс Маркета"]').element.value).toBe('250')
    await wrapper.get('input[aria-label="Автовыдача через поставщика для Яндекс Маркета"]').setValue(true)
