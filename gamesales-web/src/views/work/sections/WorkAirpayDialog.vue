<template>
  <teleport to="body">
    <div class="work-page work-modal-root modal-backdrop airpay-dialog-backdrop" :class="{ 'airpay-dialog-backdrop--purchase': purchase }" @click.self="close" @keydown="onKey">
      <section ref="dialog" class="modal modal--auto airpay-dialog" :class="{ 'airpay-dialog--purchase': purchase }" role="dialog" aria-modal="true" :aria-label="title" tabindex="-1">
        <header class="modal__head panel__head panel__head--tight">
          ...
          <button v-if="purchase" class="btn btn--icon-plain btn--icon-round deal-create-action-btn deal-create-action-btn--close" type="button" :disabled="busy" aria-label="Закрыть подтверждение покупки" title="Закрыть" @click="close"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 6l12 12M18 6l-12 12" /></svg></button>
...
const props = defineProps({ title: { type: String, required: true }, busy: { type: Boolean, default: false }, purchase: { type: Boolean, default: false } })
