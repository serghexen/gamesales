            ...
            <table class="table table--compact">
              ...
              <tbody>
                ...
                <tr v-for="item in pagedSalesHistory" :key="item.purchaseId || `${item.serviceId}-${item.createdAt}-${item.giftCode}`">
                  ...
                  <td data-label="Дата">{{ formatHistoryDate(item.createdAt) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
...
@media (max-width: 680px) {
  ...
  .interhub-confirm__details dd { text-align: left; }
  .work-page.work-modal-root.modal-backdrop.interhub-history-backdrop { align-items: stretch; padding: 8px; overflow: hidden; }
  .work-page.work-modal-root.modal-backdrop.interhub-history-backdrop .interhub-history { width: 100%; height: calc(100dvh - 16px); max-height: none; padding: 12px; }
  .interhub-history__head h3 { font-size: 20px; }
  .interhub-history__sources { display: grid; grid-template-columns: 1fr 1fr; }
  .interhub-history__sources .ghost { width: 100%; min-width: 0; }
  ...
  .interhub-history__pagination .ghost { flex: 1; }
}
</style>
