    ...
    <div v-if="showYandexMarketCatalog" class="work-page work-modal-root modal-backdrop" @click.self="closeYandexMarketCatalog">
      <div class="modal modal--auto yandex-catalog-modal">
        ...
        <div class="modal__body" :class="{ 'modal__body--locked': isBusy, 'modal__body--loader': isBusy }">
          ...
          <table v-if="!yandexMarketCatalogLoading && visibleCatalogItems.length" class="table table--compact table--dense yandex-catalog-modal__table"><thead><tr><th>Карточка Яндекс Маркета</th><th class="yandex-catalog-modal__action-head">Действие</th></tr></thead><tbody><tr v-for="item in pagedCatalogItems" :key="item.offer_id" class="clickable-row" @click="openYandexMarketCatalogDetails(item)"><td><strong>{{ item.title || item.offer_id }}</strong><div class="muted">SKU: {{ item.offer_id }}</div></td><td class="yandex-catalog-modal__action"><button class="ghost yandex-catalog-modal__open-btn" type="button" :disabled="Boolean(yandexMarketCatalogItemActionId)" @click.stop="updateYandexMarketCatalogArchive(item, !item.archived)">{{ yandexMarketCatalogItemActionId === item.offer_id ? 'Сохраняем…' : item.archived ? 'Восстановить' : 'В архив' }}</button></td></tr></tbody></table>
...
const props = defineProps({ showYandexMarketCatalog: { type: Boolean, required: true }, closeYandexMarketCatalog: { type: Function, required: true }, syncYandexMarketCatalog: { type: Function, required: true }, updateYandexMarketCatalogArchive: { type: Function, required: true }, openYandexMarketCatalogDetails: { type: Function, required: true }, yandexMarketCatalogItems: { type: Array, required: true }, yandexMarketCatalogLoading: { type: Boolean, required: true }, yandexMarketCatalogSyncing: { type: Boolean, required: true }, yandexMarketCatalogItemActionId: { type: String, default: '' }, yandexMarketCatalogError: { type: String, default: '' }, yandexMarketCatalogOk: { type: String, default: '' }, yandexMarketStoreCodes: { type: Array, default: () => [] }, yandexMarketStoreCode: { type: String, default: 'test' }, selectYandexMarketStore: { type: Function, default: () => {} } })
