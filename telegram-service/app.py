...
async def messages(payload: MessagesIn, x_api_key: str | None = Header(None)):
    ...
    try:
        ...
        req: dict[str, int] = {
            ...
            # Prevent Telethon internal None/int comparisons when paging by max_id.
            "offset_id": 0,
        }
        if payload.min_id is not None:
            min_id = int(payload.min_id)
            if min_id > 0:
                req["min_id"] = min_id
        if payload.max_id is not None:
            max_id = int(payload.max_id)
            if max_id > 0:
                req["max_id"] = max_id

        msgs = await client.get_messages(entity, **req)
        items = []
        for m in msgs:
            sender = m.sender
            if sender is None:
